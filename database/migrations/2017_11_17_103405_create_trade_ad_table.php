...
class CreateTradeAdTable extends Migration {
	...
	public function up()
	{
		Schema::create('trade_ad', function(Blueprint $table)
		{
			...
			$table->string('country_code',10)->unique()->comment('所在国家编号');
			$table->decimal('margin', 4, 2)->comment('溢价');
			$table->decimal('price', 10)->comment('价格');
			$table->decimal('min_price', 10)->comment('最低价格');
			$table->integer('min_amount')->comment('最小限额');
			$table->integer('max_amount')->comment('最大限额');
			$table->integer('payment_window_minutes')->comment('付款期限');
			$table->string('payment_provider')->comment('付款方式');
			$table->text('message', 65535)->comment('广告留言');
